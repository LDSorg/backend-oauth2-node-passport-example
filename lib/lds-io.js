...
module.exports.create = function (app, passport, conf) {
  ...
  // support oauth2 convention via :providerUri
  app.get(authorizationRedirect + '/:providerUri', passport.authenticate('oauth3-strategy-1'));
  ...
  function handleOauth3Response(req, res, next, err, user, info/*, status*/) {
    ...
    return new PromiseA(function (resolve, reject) {
      req.login({ profile: user, info: info }, function (err) {
        if (err) {
          params.error = err.message;
          params.error_description = err.message || err;
          reject(params);
          return;
        }
        ...
        params.state = info.browserState || info.state;

        // don't mistake undefined as exists, which would be truthy
  ...
  app.get(accessTokenCallback, function (req, res, next) {
    passport.authenticate('oauth3-strategy-1', function (err, user, info, status) {
      ...
      handleOauth3Response(req, res, next, err, user, info, status).then(function (params) {
        var url = '/oauth3.html#' + querystring.stringify(params);
        ...
        if (err.error_description) {
          params = err;
          url = '/oauth3.html#' + querystring.stringify(params);
          res.redirect(url);
        } else {
